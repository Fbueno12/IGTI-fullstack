...
function findUsers(text, users) {
  ...
  const filtered = users.filter(value => {
    const first = removeSpecial(value.name.first.toLowerCase());
    const last  = removeSpecial(value.name.last.toLowerCase());
...
function removeSpecial(text) {
  ...
  return text.replace(/[\W\[\] ]/g,function(a){return map[a]||a});
}